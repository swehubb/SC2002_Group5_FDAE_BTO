...
private void viewReceiptInterface()) { //Visible option in the main applicant menu
    ...
    if (application != null && application.getStatus() == ApplicationStatus.BOOKED) {
        // Pass the BookingController when generating the receipt
        String receipt = currentApplicant.generateReceipt(bookingController);
        System.out.println(receipt);
        
        // Check if a receipt has been stored by an officer
        if (bookingController.hasReceipt(currentApplicant.getNric())) {
            System.out.println("\nNote: To save this receipt, you can copy and paste the text above.");
        } else {
            System.out.println("\nThis is a temporary receipt. Please contact an HDB Officer to generate an official receipt.");
        }
    } else {
...
private void viewBookingReceipt(Applicant applicant) { //Called in Application Status Window: If flat is booked Book Receipt will be displayed
    ...
    // If a receipt exists in the BookingController, show save option
